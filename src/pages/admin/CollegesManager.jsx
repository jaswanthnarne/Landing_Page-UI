...
export default function CollegesManager() {
    ...
    const addEducationalPartner = useWebsiteStore((state) => state.addEducationalPartner);
    const deleteEducationalPartner = useWebsiteStore((state) => state.deleteEducationalPartner);
    ...
    const handleImageUpload = async (e) => {
        ...
        setIsUploading(true);
        try {
            const uploadedUrl = await uploadToCloudinary(file, 'colleges');
    ...
    const handleAddCollege = (e) => {
        e.preventDefault();
        if (!logoUrl) {
            alert('Please upload a college logo first.');
            return;
        }

        addEducationalPartner({ name, logo: logoUrl });
        setName('');
        setLogoUrl('');
        setIsOpen(false);
    };
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-[14.5px] font-extrabold text-slate-800">Educational Partners</h2>
                    <p className="text-[11px] text-slate-400 font-semibold tracking-wider uppercase mt-1">Configure partner colleges and university logos</p>
                </div>
                <button
                    onClick={() => {
                        setName('');
                ...
                {educationalPartners.map((partner, index) => (
                    ...
                    >
                        ...
                        <button
                            onClick={() => {
                                if (confirm(`Are you sure you want to delete "${partner.name}"?`)) {
                                    deleteEducationalPartner(index);
                                }
                            }}
                        ...
                        <div className="w-full border-t border-slate-50 pt-2 mt-2">
                            <span className="text-[11px] font-bold text-slate-700 block truncate leading-tight">
                                {partner.name}
                            </span>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="bg-white border border-slate-100 rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden relative z-10">
                        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                            <h3 className="font-extrabold text-[15px] text-slate-800">Add New Educational College</h3>
                            <button
                                onClick={() => setIsOpen(false)}
                        ...
                        <form onSubmit={handleAddCollege} className="p-6 space-y-4">
                            <div className="space-y-1">
                                <label className="text-[12px] font-bold text-slate-700 uppercase tracking-wide">College Name</label>
                                <input
                                    type="text"
                                    required
                                    value={name}
                            ...
                            {/* Cloudinary upload for college logo */}
                            <div className="space-y-2">
                                <label className="text-[12px] font-bold text-slate-700 uppercase tracking-wide block">College Logo Image</label>
                            ...
                            <div className="space-y-1">
                                ...
                                <input
                                    ...
                                    placeholder="e.g. /Partners/Sona.png or Cloudinary URL"
                                />
                            </div>
